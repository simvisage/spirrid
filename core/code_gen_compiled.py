...
from enthought.traits.api import \
    HasStrictTraits, Array, Property, Float, cached_property, Callable, Tuple, \
    List, Str, Enum, Int, Instance, Trait, WeakRef, Bool, on_trait_change, Dict, \
    Interface, implements, Event
...
class CodeGenCompiled(CodeGen):
    ...
    def _get_rand_var_idx_list(self):
        dt = np.array(map(type, self.core.tvar_lst))
    ...
    def _get_theta_arrs(self):
        ...
        theta = self.core.sampling.theta
    ...
    def _get_dG_arrs(self):
        dG_ogrid = self.core.sampling.dG_ogrid
        rv_getter = operator.itemgetter(*self.rand_var_idx_list)
    ...
    def _get_LD_N_TAB(self):
        if self.core.sampling_type == 'LHS' or self.core.sampling_type == 'MCS':
            if self.compiled_eps_loop:
                return 3
            else:
                return 2
    ...
    def _get_code(self):
        ...
        if self.compiled_eps_loop:
            ...
            #
            n_eps = len(self.core.evar_lst[0])
        ...
        for name, distr in zip(self.var_names, self.core.tvar_lst):
        ...
        inner_code_str = ''
        lang = self.ld + '_code'
        q_code = getattr(self.core.q, lang)
        import textwrap
    ...
    def get_cython_code(self):
        ...
        cython_file_name = 'core_cython.pyx'
...
class CodeGenCompiledTGrid(CodeGenCompiledRegular):
    ...
    def _get_arg_values_dG(self):
        ...
        if self.n_rand_vars > 0:
            if self.cached_dG:
                arg_values[ 'dG_grid' ] = self.core.sampling.dG
            else:
...
class CodeGenCompiledPGrid(CodeGenCompiledRegular):
    ...
    def _get_code_dG_declare(self):
        n_sim = self.core.sampling.n_sim
        if self.compiled_eps_loop:
            if self.cached_dG:
