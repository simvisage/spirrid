...
class SPIRRIDLAB(HasTraits):
    ...
    def _plot_sampling(self,
                      ...
                      xlim = None,
                      plot_idx = [0, 1]):
        '''Construct a core object, run the calculation
    ...
    def sampling_efficiency(self, n_int_range):
        ...
        def run_estimation(n_int, sampling_type):
            # instantiate core with samplingetization methods 
        ...
        if self.save_output:
            fname = os.path.join('fig', qname + '_' + '%s' % self.hostname + '_time_nint' + '.png')
            p.savefig(fname)
        ...
        if len(self.exact_arr) > 0:
            ...
            if self.save_output:
                fname = os.path.join('fig', qname + '_' + '%s' % self.hostname + '_error_nint' + '.png')
                p.savefig(fname)
            ...
            if self.save_output:
                fname = os.path.join('fig', qname + '_' + '%s' % self.hostname + '_error_time' + '.png')
                p.savefig(fname)
...
#                 )
    ...
    def codegen_efficiency(self):
        ...
        for idx, run in enumerate(self.run_lst_detailed_config):
            ...
            for i in range(self.n_recalc):
                s.recalc = True # automatically proagated within core
    ...
    def codegen_language_efficiency(self, extra_compiler_args = True,
        ...
        for extra in [extra_compiler_args]:
            ...
            meth_lst = zip(sampling_lst, n_int_lst)
